// stb_truetype.h - v0.3 - public domain - 2009 Sean Barrett / CMD Game Tools
//
...
void my_stbtt_print(float x, float y, char *text)
{
   ...
   while (*text) {
      if (*text >= 32 && *text < 128) {
         ...
         stbtt_GetBakedQuad(cdata, 512,512, *text-32, &x,&y,&q,1,1);//1=opengl,0=old d3d
...
extern int stbtt_BakeFontBitmapWithChars(const unsigned char *data, int offset,  // font location (use offset=0 for plain .ttf)
                                float pixel_height,                     // height of font in pixels
                                unsigned char *pixels, int pw, int ph,  // bitmap to be filled in
								int first_char, int num_chars,          // range of characters to bake
                                const char * characters,          // characters to actually bake
...
extern void stbtt_GetBakedQuad(stbtt_bakedchar *chardata, int pw, int ph,  // same data as above
							   ...
							   int increment); // increment 1 or -1 or whatever
...
//
   ...
   enum {
      ...
      STBTT_vcurve
                   ...
                   // (we share this with other code at CMD)
                                ...
                                stbtt_bakedchar *chardata)
{
   ...
   for (i=0; i < slen; ++i) {
	  ...
	  int c = characters[i]-first_char;
      int g = stbtt_FindGlyphIndex(&f, characters[i]);
	  ...
	  chardata[c].baked = 1;
      chardata[c].x0 = (stbtt_int16) x;
      chardata[c].y0 = (stbtt_int16) y;
      chardata[c].x1 = (stbtt_int16) (x + gw);
      chardata[c].y1 = (stbtt_int16) (y + gh);
      chardata[c].xadvance = scale * advance;
      chardata[c].xoff     = (float) x0;
      chardata[c].yoff     = (float) y0;
      x = x + gw + 2;
      if (y+gh+2 > bottom_y)
         bottom_y = y+gh+2;
   }
...
void stbtt_GetBakedQuad(stbtt_bakedchar *chardata, int pw, int ph, int char_index, float *xpos, float *ypos, stbtt_aligned_quad *q, int opengl_fillrule, int increment )
{
