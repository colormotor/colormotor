...
Vec3 sphericalToCartesian( const Vec3 & thetaPhiR )
{
    float theta = thetaPhiR.x;
    float phi = thetaPhiR.y;
    float r = thetaPhiR.z;
...
M33 reflection2d( const Vec2 & a, const Vec2 & b )
{
    M33 out;
    out.identity();
...
M33 getFittingMatrix( const Rectf & src, const Rectf & dst, bool bCenter )
{
    ...
    m.translate(fcenp.x-cenp.x,fcenp.y-cenp.y);//(fitted.l-src.l),(fitted.t-src.t),0);
    m.translate(cenp.x,cenp.y);
    m.scale(fitted.getWidth()/src.getWidth(),fitted.getHeight()/src.getHeight());	
    m.translate(-cenp.x,-cenp.y);
    return m;
