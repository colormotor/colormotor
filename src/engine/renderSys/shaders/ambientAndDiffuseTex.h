...
const char *vsAmbientDiffuseTex = STRINGIFY(
...
const char * psAmbientDiffuseTex = STRINGIFY(
...
void main( void )
{	
	vec4 c = clr*diffuse*texture2D(tex,uv)*contrast;
	float nl = max( dot( normalize(l),normalize(n) ), diffuseMin );
	vec4 a = ambient * diffuse * luminosity( getNormalColor( normalize(n) ) ) * ambientAmt;
	vec4 d = c * nl;
	c = a+d; 
