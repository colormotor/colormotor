...
%fragment( "armanpy_mat_typemaps", "header", fragment="armanpy_typemaps" )
{
    ...
    bool armanpy_numpy_as_mat_with_shared_memory( PyObject* input, MatT **m )
    {
        ...
        if( armanpy_allow_conversion_flag )
        {
            array = obj_to_array_fortran_allow_conversion( input, NumpyType<eT>::val, &is_new_object );
            if(armanpy_warn_on_conversion_flag && is_new_object)
                printf("armanpy_numpy_as_mat_with_shared_memory 2d conv\n");
        }
        else
        {
            array = obj_to_array_no_conversion( input, NumpyType<eT>::val );    
        ...
        *m = new MatT( (eT*)array_data(array), r, c, is_new_object, false );
...
%define %armanpy_mat_byvalue_typemaps( ARMA_MAT_TYPE )
        ...
        (       ARMA_MAT_TYPE   ) ( PyArrayObject* array=NULL )
    {
        ...
        if( armanpy_allow_conversion_flag )
        {
            ...
            if(armanpy_warn_on_conversion_flag && is_new_object)
                printf("armanpy_mat_byvalue_typemaps 2d conv\n");
            if( !array ) SWIG_fail;
            $1 = ARMA_MAT_TYPE( ( ARMA_MAT_TYPE::elem_type *)array_data(array), array_dimensions(array)[0], array_dimensions(array)[1], true );
...
%define %armanpy_mat_const_ref_typemaps( ARMA_MAT_TYPE )
        ...
        ( const ARMA_MAT_TYPE * ) ( PyArrayObject* array=NULL )
    {
        ...
        if( armanpy_allow_conversion_flag )
        {
            ...
            if(armanpy_warn_on_conversion_flag && is_new_object)
            {
                printf("armanpy_mat_const_ref_typemaps 2d conv\n");
                // printf("Dim: %d %d\n",(int)array_dimensions(array)[0], (int)array_dimensions(array)[1] );
                // int rows = (int)array_dimensions(array)[0];
                // int cols = (int)array_dimensions(array)[1];
                // ARMA_MAT_TYPE::elem_type * data = ( ARMA_MAT_TYPE::elem_type *)array_data(array);
                // printf("[");
            ...
            $1 = new ARMA_MAT_TYPE( ( ARMA_MAT_TYPE::elem_type *)array_data(array),
                                arma::uword( array_dimensions(array)[0] ), arma::uword( array_dimensions(array)[1] ), true, false );
        ...
        else
        {
            ...
            if( !array ) SWIG_fail;
        $1 = new ARMA_MAT_TYPE( ( ARMA_MAT_TYPE::elem_type *)array_data(array),
                                arma::uword( array_dimensions(array)[0] ), arma::uword( array_dimensions(array)[1] ), false, true );
        }
